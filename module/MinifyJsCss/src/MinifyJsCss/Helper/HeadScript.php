...
class HeadScript extends ZendViewHelperHeadScript {
	...
	public static function getMe(Url $pUrl){
		...
		$hdLink->setMinifyUrlBase($pUrl('minify'));
		return $hdLink;
	}
	...
	public function ksort(){
		$arrCopy=$this->getContainer()->getArrayCopy();
		foreach($arrCopy as $x=>$itm) $fArr[$itm->groupIndex][$x]=$x;
		ksort($fArr, SORT_NUMERIC);
		foreach($fArr as $x=>$aX){
			foreach($aX as $k) $vArr[]=$arrCopy[$k];
		}
		#die('<pre>'.var_export($vArr, true));
		$this->getContainer()->exchangeArray($vArr);
		#return $this->getContainer()->uksort(function ($a, $b){ return intval($a)>intval($b); });
	}
    ...
    public function createData($type, array $attributes, $content = null) {
    	if(array_key_exists('groupIndex', $attributes)){
    		$idx2sort=$attributes['groupIndex'];
    		unset($attributes['groupIndex']);
    	}else $idx2sort=0;
