        ...
        foreach ($parameters as $position => $paramsForPosition) {
            if (isset($paramSpecs[$position]['combinedby'])) {
                ...
                foreach ($paramsForPosition as $multiParamsForPosition) {
                    $ppCount = (is_scalar($multiParamsForPosition)?(is_null($multiParamsForPosition)?0:1):count($multiParamsForPosition));
                ...
                $ppCount = (is_scalar($paramsForPosition)?(is_null($paramsForPosition)?0:1):count($paramsForPosition));
