 ...
 */
class Openssl implements \Zend\Crypt\Symmetric\SymmetricInterface
{
    ...
    public function __construct($options = [])
    {
        if (! extension_loaded('openssl')) {
        		throw new \Zend\Crypt\Symmetric\Exception\RuntimeException(sprintf(
    ...
    public function setOptions($options)
    {
        ...
        if (! is_array($options)) {
        		throw new \Zend\Crypt\Symmetric\Exception\InvalidArgumentException(
    ...
    public static function setPaddingPluginManager($plugins)
    {
        if (is_string($plugins)) {
        		if (! class_exists($plugins) || ! is_subclass_of($plugins, \Zend\ServiceManager\ServiceLocatorAwareInterface::class)) {
        			throw new \Zend\Crypt\Symmetric\Exception\InvalidArgumentException(sprintf(
        ...
        if (! $plugins instanceof \Zend\ServiceManager\ServiceLocatorAwareInterface) {
        		throw new \Zend\Crypt\Symmetric\Exception\InvalidArgumentException(sprintf(
    ...
    public function setKey($key)
    {
        ...
        if (! $keyLen) {
        		throw new \Zend\Crypt\Symmetric\Exception\InvalidArgumentException('The key cannot be empty');
        }
    ...
    public function setAad($aad)
    {
        if (! $this->isAuthEncAvailable()) {
        		throw new \Zend\Crypt\Symmetric\Exception\RuntimeException(
    ...
    public function encrypt($data)
    {
        ...
        if (! is_string($data) || $data === '') {
        		throw new \Zend\Crypt\Symmetric\Exception\InvalidArgumentException('The data to encrypt cannot be empty');
        }

        if (null === $this->getKey()) {
        		throw new \Zend\Crypt\Symmetric\Exception\InvalidArgumentException('No key specified for the encryption');
        }

        if (null === $this->getSalt() && $this->getSaltSize() > 0) {
        		throw new \Zend\Crypt\Symmetric\Exception\InvalidArgumentException('The salt (IV) cannot be empty');
        }

        if (null === $this->getPadding()) {
        		throw new \Zend\Crypt\Symmetric\Exception\InvalidArgumentException('You must specify a padding method');
        }
    ...
    {
        if (empty($data)) {
        		throw new \Zend\Crypt\Symmetric\Exception\InvalidArgumentException('The data to decrypt cannot be empty');
        }

        if (null === $this->getKey()) {
        		throw new \Zend\Crypt\Symmetric\Exception\InvalidArgumentException('No decryption key specified');
        }
        if (null === $this->getPadding()) {
        		throw new \Zend\Crypt\Symmetric\Exception\InvalidArgumentException('You must specify a padding method');
        }
